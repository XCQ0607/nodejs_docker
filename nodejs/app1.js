...
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
console.log("Cloudflare一键无交互Vless代理脚本 + xbin上传功能");
console.log("当前版本：V1.1.0");
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
...
<body>
    ...
    httpServer.listen(PORT, () => {
        ...
        const deploymentInfo = {
            ...
            domain: DOMAIN,
            port: PORT,
            selectUrl: `http://${DOMAIN}:${PORT}/${UUID}/select`,
            configUrl: `http://${DOMAIN}:${PORT}/${UUID}`,
        ...
        // 异步上传，不阻塞服务器启动
        uploadToXbin(deploymentInfo).catch(error => {
...
async function uploadToXbin(deploymentInfo) {
    if (!BINURL || !BINPATH || !BINPWD) {
        console.log('xbin 上传配置不完整，跳过上传');
        return;
    }

    try {
        console.log('开始上传部署信息到 xbin...');
        ...
        if (response.status === 200) {
            console.log('✅ 部署信息上传成功到 xbin');
            console.log('上传响应:', response.data);
        } else {
            console.log('❌ xbin 上传失败，状态码:', response.status);
        }
    } catch (error) {
        console.error('❌ xbin 上传失败:', error.message);
    }
}
